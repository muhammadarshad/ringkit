...
| C9 | **The exact identity is never quantized to save bits, and no Euclidean/standard-domain form is imported to define it.** The ARC/phase position (exact additive RoPE, exact Δtick, exact quadrant) is carried at FULL precision, always. The ring-native KV element is **ADI (accumulation, differential)** — `(x,y)→(x+y, x−y)`, exact/reversible/multiplier-free — NOT the Euclidean polar form (`atan2` + `sqrt(x²+y²)`), which is a lossy anti-pattern that forced SIN/COS reconstruction and was **removed**. A codec may quantize only genuinely redundant magnitude/energy — never the arc — and never to beat a benchmark's bit-count. | HELD — `ml/kvadi.py` is the exact lossless ADI element (`tests/test_kvadi.py`: bit-exact round-trip over all 65,536 pairs, no isqrt/atan/sqrt, multiplier-free). The Euclidean polar codec + its magnitude quantizer were removed (an earlier arc-quantizing "3.5 b/coord < PolarQuant" draft was rejected as a violation). Compression on ADI is an OPEN, form-first step gated by this rule. |
