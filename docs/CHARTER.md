...
| D9 | **Two layers: semantic vs silicon.** The multiplier-free rule (C4, no `*`/`//`/`**`) governs the ring **semantic** layers (core, stats, calculus, linalg, array, physics, ml) — they are the correct reference. The `kernels/` **silicon** layer is the designated exception: it uses hardware ops (`*`, SIMD) on purpose, to trade ALU cycles for speed, and must reproduce the semantic layer **bit-for-bit** (cross-checked in tests) before it is trusted. Hardware ops in `kernels/` are not a violation; they are the point of that layer. The AST audit excludes `kernels/`. |
